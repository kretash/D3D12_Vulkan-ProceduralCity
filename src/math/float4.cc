...
float4x4::float4x4( float4 x, float4 y, float4 z, float4 w ) {
  float tmp[16]{
    x.x, x.y, x.z, x.w,
    y.x, y.y, y.z, y.w,
    z.x, z.y, z.z, z.w,
...
void float4x4::translate( float3 p ) {
  data[12] = p.x;
  data[13] = p.y;
  data[14] = p.z;
}

void float4x4::rotate_x( float a ) {
  data[5] = cosf( a );
  data[6] = -sinf( a );
  data[9] = sinf( a );
  data[10] = cosf( a );
...
void float4x4::rotate_z( float a ) {
  data[0] = cosf( a );
  data[1] = -sinf( a );
  data[4] = sinf( a );
  data[5] = cosf( a );
...
void float4x4::scale( float3 scale ) {
...
void float4x4::look_at( float3 eye, float3 focus, float3 up ) {
  ...
  data[0] = x_axis.x; data[1] = y_axis.x; data[2] = z_axis.x; data[3] = 0;
  data[4] = x_axis.y; data[5] = y_axis.y; data[6] = z_axis.y; data[7] = 0;
  data[8] = x_axis.z; data[9] = y_axis.z; data[10] = z_axis.z; data[11] = 0;

  data[12] = -float3::dot( x_axis, eye ); data[13] = -float3::dot( y_axis, eye );
  data[14] = -float3::dot( z_axis, eye ); data[15] = 1;
...
float4x4 float4x4::inverse( float4x4 m ) {
