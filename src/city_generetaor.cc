...
namespace kretash {
  ...
  void CityGenerator::update() {
    ...
    static int count = 0;
    bool update_city = k_engine_settings->get_psettings()->update_city;

    if( m_move_operations.size() == 0 && count == 1 && update_city ) { //0.1 -> 0.5, spikes to 1.7
      _prepare_vectors();
